...
const GamePage = ({ gameData, tagData, devData, params }) => {
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:title" content={`${game.name} 🕹️ | For Free Online! 🐇`} />
        ...
        <meta property="og:url" content={`https://playem.io/app/${params.id}`} />
            ...
            __html: JSON.stringify([
              ...
              },
              {
                ...
                "itemListElement": [
                  ...
                  { "@type": "ListItem", "position": 2, "name": tags[0].name, "item": `https://playem.io/tag/${game.tags[0]}` }
                ]
              }
            ])
          }}
      ...
      <center>
        ...
        <div className="descriptionBubble">
          <h2>{game.name}</h2>
          <div>
            <div style={{ display: 'inline-block', marginLeft: '30px', cursor: 'pointer' }} onClick={() => window.open(`/play/${params.id}`, '_blank')}>
              <img src={`/img/${params.id}.jpg`} width="220px" height="136px" style={{ borderRadius: '12px' }} alt="Play Now" />
            </div>
                ...
                <div style={{ display: 'inline-block' }}>
                  <ul style={{ padding: 0, margin: 0, listStyleType: 'none' }}>
                    {game.ratingValue && game.ratingCount ? (
                      <li>{game.ratingValue} <span style={{ fontSize: '12px' }}>({game.ratingCount} votes)</span></li>
                    ) : (
                      <li>N/A</li>
                    )}
                    {dev ? (
                      <li><a href={`/dev/${game.dev}`}>{dev.name}</a></li>
                    ) : game.dev ? (
                      <li>{game.dev}</li>
                    ) : (
                      <li>N/A</li>
            ...
            <div style={{ margin: '12px 30px' }}>
              {game.desc || `${game.name} is available for free on Playem.io. Try it out or check out thousands of similar apps on Playem.io.`}
            </div>
            <div style={{ margin: '22px 30px 0 30px' }}>
              {game.tags.map((tagId, index) => (
                tagData[tagId] && (
                  <a key={index} href={`/tag/${tagId}`}>
                    <div className="descriptionBubble-gameTag">{tagData[tagId].name}</div>
                  </a>
                )
              ))}
            </div>
        ...
        <div style={{ display: 'inline-block', verticalAlign: 'top', marginBottom: '36px', width: '80%' }}>
        ...
        <div className="descriptionBubble">
          ...
          <div style={{ margin: '0 30px' }}>
            {game.howToPlay || `${game.name} is available for free online on Playem.io. You can download and install it, use it unblocked, or play it instantly on browser.`}
          </div>
          <h2>What's New</h2>
          <div style={{ margin: '0 30px' }}>
            {game.updateNotes || 'N/A'}
          </div>
          <h2>About The Developer</h2>
          <div style={{ margin: '0 30px' }}>
            {dev && dev.desc ? dev.desc : `${dev ? dev.name : game.dev} developed the software.`}
          </div>
          <script
            type="application/ld+json"
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                ...
                "mainEntity": [
                  ...
                  },
                  {
                    ...
                    "acceptedAnswer": {
                      "@type": "Answer",
                      "text": `${dev ? dev.name : game.dev} developed the software.`
                    }
                  },
                  {
                    "@type": "Question",
        ...
        dangerouslySetInnerHTML={{
          __html: `
            ...
            if (${JSON.stringify(dev)}) {
              updateGameList('rating', 'dev', '${game.dev}', 'sameDeveloperList', 'short');
            }
            ${game.tags.map((tagId, index) => `
              document.getElementById('gameList').innerHTML += '<div id="gameList${index + 1}"></div>';
              updateGameList('rating', 'tag', '${tagId}', 'gameList${index + 1}', 'short');
            `).join('')}
